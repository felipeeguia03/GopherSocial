...
func (s *UsersStore) Create(ctx context.Context, tx *sql.Tx, user *User) error {
	...
	err := tx.QueryRowContext(ctx, query, user.Username, user.Email, user.Password.hash).Scan(
		&user.ID,
		&user.CreatedAt,
	)
	if err != nil {
...
func (s *UsersStore) CreateAndInvite(ctx context.Context, user *User, token string, invitationExp time.Duration) error {
	return withTx(s.db, ctx, func(tx *sql.Tx) error {
		...
		if err := s.createUserInvitation(ctx, token, tx, user.ID, invitationExp); err != nil {
			return err
		}
		return nil
	})
...
func (s *UsersStore) createUserInvitation(ctx context.Context, token string, tx *sql.Tx, userID int64, invitationExp time.Duration) error {
...
func (s *UsersStore) getUserFromInvitation(ctx context.Context, tx *sql.Tx, plainToken string) (*User, error) {
